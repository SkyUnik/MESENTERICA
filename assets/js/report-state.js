(() => {
  ...
  const elements = {
    ...
    guidanceBox: document.querySelector('.report-guidance'), guidanceEyebrow: document.getElementById('report-guidance-eyebrow'), guidanceTitle: document.getElementById('report-guidance-title'), guidanceSummary: document.getElementById('report-guidance-summary'), guidancePoints: document.getElementById('report-guidance-points'),
  ...
  function renderGuidance(item, detection) {
    ...
    const guidance = window.MesentericaClinical.getGuidance(system, guidanceTop, detection); elements.guidanceBox.dataset.guidanceColor = guidance.color; elements.guidanceEyebrow.textContent = guidance.eyebrow; elements.guidanceTitle.textContent = guidance.title; elements.guidanceSummary.textContent = guidance.summary; elements.guidancePoints.replaceChildren();
  ...
  function renderCase(index) {
    ...
    const acceptedCells = system?.counts?.acceptedSpeciesCells || 0; const candidateCells = system?.counts?.candidateSpeciesCells || 0;
    elements.resultTitle.textContent = isV3 ? system.outcome.message : top.presentation; elements.resultDetail.textContent = detection.description; elements.detectionBadge.textContent = `Status: ${detection.label}`; elements.topScore.textContent = isV3 ? (acceptedCells ? `${acceptedCells} sel diterima` : (candidateCells ? `${candidateCells} kandidat spesies · belum diterima` : '0 sel diterima')) : `${(top.score * 100).toFixed(1)}%`;
    ...
    const appliedThreshold = system?.thresholds?.detectorCandidate; const defaultThreshold = system?.thresholds?.detectorDefault; const thresholdOrigin = Number.isFinite(appliedThreshold) && Number.isFinite(defaultThreshold) && Math.abs(appliedThreshold - defaultThreshold) > 1e-9 ? 'diubah pengguna' : 'default model';
    elements.threshold.textContent = isV3 ? (Number.isFinite(appliedThreshold) ? `${appliedThreshold.toFixed(6).replace('.', ',')} · ${thresholdOrigin}; classifier per kelas dari metadata` : 'Threshold detector dan per-kelas dari metadata versi model') : `${(batch.threshold * 100).toFixed(0)}% · threshold tampilan non-tervalidasi`; elements.detectionStatus.textContent = `${detection.label} · ${detection.description}`;
