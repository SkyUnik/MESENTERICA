...
function decodeDetector(data, metadata, left, top, sourceWidth, sourceHeight) {
  const count = metadata.output.shape[2]; const size = metadata.tileSize; const candidates = [];
  for (let index = 0; index < count; index += 1) {
    const confidence = data[4 * count + index]; if (confidence < metadata.candidateThreshold) continue;
...
function aggregate(detectorBoxes, classifierOutputs, thresholds) {
  if (!detectorBoxes.length) return { outcomeCode: 'no_parasite_detected', message: 'No parasite detected in this image', reason: 'zero accepted detector boxes', primarySpecies: null, secondarySpecies: [], acceptedSpeciesCells: 0, rejectedCells: 0, speciesCounts: {}, speciesEvidence: Object.fromEntries(SPECIES.map((name) => [name, 0])), cells: [] };
  ...
  const accepted = cells.filter((cell) => cell.acceptedSpecies); const counts = {}; const scores = Object.fromEntries(SPECIES.map((name) => [name, 0]));
  ...
  const total = Object.values(scores).reduce((sum, value) => sum + value, 0); const evidence = Object.fromEntries(SPECIES.map((name) => [name, total ? scores[name] / total : 0]));
  const dominant = [...SPECIES].sort((a, b) => scores[b] - scores[a])[0]; const base = { cells, acceptedSpeciesCells: accepted.length, rejectedCells: cells.length - accepted.length, speciesCounts: counts, speciesEvidence: evidence };
  if (!accepted.length) return { ...base, outcomeCode: 'species_uncertain', message: 'Uncertain—manual review required', reason: 'detector boxes were present but every classifier output was rejected', primarySpecies: null, secondarySpecies: [] };
...
export async function loadTwoStage(manifestUrl = new URL('model/manifest.json', document.baseURI).href, options = {}) {
  ...
  async function predict(source) {
    ...
    for (const top of tilePositions(height, detectorMetadata.tileSize, detectorMetadata.tileOverlap)) for (const left of tilePositions(width, detectorMetadata.tileSize, detectorMetadata.tileOverlap)) {
      ...
      const output = await runModel(detectorModel, imageDataToNchw(tileContext.getImageData(0, 0, detectorMetadata.tileSize, detectorMetadata.tileSize)), detectorMetadata.input.shape); detectorBoxes.push(...decodeDetector(output, detectorMetadata, left, top, width, height));
    }
    ...
    return { schemaVersion: 'mesenterica-case-result-v3', runtime: { name: 'LiteRT.js', version: manifest.runtime.version, accelerator, mixedExecution, fallbackReason }, timing: { detectorMilliseconds, classifierMilliseconds: performance.now() - classifierStarted, totalMilliseconds: performance.now() - totalStarted, tileCount }, detectorBoxes: deduplicated, cells: aggregation.cells, speciesEvidence: aggregation.speciesEvidence, outcome: { code: aggregation.outcomeCode, message: aggregation.message, reason: aggregation.reason, primarySpecies: aggregation.primarySpecies, secondarySpecies: aggregation.secondarySpecies }, counts: { detectorBoxes: deduplicated.length, acceptedSpeciesCells: aggregation.acceptedSpeciesCells, rejectedCells: aggregation.rejectedCells, species: aggregation.speciesCounts }, versions: { manifest: manifest.manifestVersion, detector: detectorMetadata.modelVersion, classifier: classifierMetadata.modelVersion, aggregation: manifest.aggregationVersion, preprocessing: 'nchw-rgb-reflect101-v1', thresholds: 'group-safe-provisional-v1' } };
  }
  function drawBoxes(source, result, options = {}) {
    ...
    for (const cell of result?.cells || []) { const className = cell.acceptedSpecies ? cell.predictedClass : 'uncertain'; const color = BOX_COLORS[className] || BOX_COLORS.uncertain; const x1 = cell.x1 * scale; const y1 = cell.y1 * scale; const x2 = cell.x2 * scale; const y2 = cell.y2 * scale; context.strokeStyle = color; context.lineWidth = clamp(canvas.width / 350, 2, 6); context.strokeRect(x1, y1, x2 - x1, y2 - y1); const probability = cell.probabilities?.[cell.predictedClass] || 0; const label = cell.acceptedSpecies ? `infected cell · P. ${cell.predictedClass} ${(probability * 100).toFixed(1)}%` : `candidate · ${cell.predictedClass || 'review'}`; const padding = 5; const labelWidth = context.measureText(label).width + padding * 2; const fontSize = Number(context.font.match(/([0-9.]+)px/)?.[1] || 14); const labelHeight = fontSize + padding * 2; const labelY = y1 >= labelHeight ? y1 - labelHeight : y1; context.fillStyle = color; context.fillRect(x1, labelY, labelWidth, labelHeight); context.fillStyle = '#fff'; context.fillText(label, x1 + padding, labelY + padding); }
    return canvas;
  }
